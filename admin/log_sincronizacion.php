...
class LogSincronizacionTable extends Obuma_List_Table {
  ...
  public function __construct(){
    parent::__construct(array(
            'singular' => "obuma_log_synchronization",
            'plural' => "obuma_log_synchronization",
            'ajax' => true
            ));
...
public function get_sql_results(){
    ...
    $args = array('id','fecha','hora','tipo','opcion','resultado');
    $sql_select = implode(', ', $args);
    $sql = "";
    $sql .= "SELECT " . $sql_select . " FROM " . $wpdb->prefix . "obuma_log_synchronization";

    if($search){
    	$sql .= " WHERE id  LIKE '%".$search."%'  OR fecha LIKE '%".$search."%' OR tipo LIKE '%".$search."%'  OR opcion LIKE '%".$search."%'  OR resultado LIKE '%".$search."%'"; 
    }
...
function my_render_list_page(){
  $myListTable = new LogSincronizacionTable();
  echo check_version();
  echo '<div class="wrap"><h2>LOG DE SINCRONIZACI&Oacute;N</h2>'; 
